...
class x10_man(Resource):
    ...
    def result(self, result):
        ...
        self.request.write(str(template.render(pluginid=self.pluginid, pluginguid=self.pluginguid, result=result)))
        self.request.finish()
    ...
    def render_GET(self, request):
        ...
        plugins = self.coordinator.get_plugins_by_type("x10")
        print "DEBUG: LEN PLUGINS=",len(plugins)
        if len(plugins) == 0:
            print "BEN IK ER?"
            self.request.write(str("No online X10 plugins found..."))
            self.request.finish()
...
class x10_added(Resource):
    ...
    def result(self, result):
        ...
        self.db.update_or_add_value("Status", "Unknown", self.data['pluginid'], self.data['hcdc'])
...
class x10_del(Resource):
    ...
    def result(self, result):
        # Notify the plugin        
        self.coordinator.send_custom(self.data['pluginguid'], "del_characteristics", {'hcdc': self.data["hcdc"]})
    ...
    def render_POST(self, request):
        ...
        self.db.del_device(self.data['id']).addCallback(self.result)
